...
data_AttitudesonGenderRoles <- read.csv("../data/data_AttitudesonGenderRoles.csv")
```
...
```{r}
data_AttitudesonGenderRoles_tidy <- data_AttitudesonGenderRoles %>%
  select(Country, Gender, Gender_ideology_Men_should_prioritize, Gender_ideology_Women_should_prioritize) %>%
  group_by(Country, Gender) %>%
  summarise(Men.Role = mean(Gender_ideology_Men_should_prioritize, na.rm=TRUE),
            Women.Role = mean(Gender_ideology_Women_should_prioritize, na.rm=TRUE),
            count = n()) %>%
  filter(count >= 5) %>%
  group_by(Country) %>%
  summarise(Men.Role = mean(Men.Role, na.rm=TRUE),
            Women.Role = mean(Women.Role, na.rm=TRUE)) %>%
  mutate(Women.More.Home = Men.Role - Women.Role) # more positive greater belief that women should stay home
```
Nevermind, this doesn't even have Singapore...
