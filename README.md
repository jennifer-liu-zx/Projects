# Projects

## Programming Projects

### Forecasting Relative Humidity with Machine Learning and Neural Networks (NUS)
- Course: IT1244 Artificial Intelligence and Society (Year 2)
- Directory: `Weather-Forecasting`
- Language: Python
- Featured methods/packages: Random Forest, XGBoost, LSTM

### Features of Successful Holiday Episodes (NUS)
- Course: DSA2101 Essential Data Analytics Tools: Data Visualisation (Year 2)
- Directory: `Successful-Holiday-Episodes`
- Language: R (RMarkdown)
- Featured methods/packages: tidyverse

### Regional Specificity in Factors Affecting Economic Growth from 1960 to 1996 (NUS)
- Course: DSE1101 Introductory Data Science for Economics (Year 1)
- Directory: `Economic-Growth-Prediction`
- Language: R
- Featured methods/packages: linear regression, K-means Clustering, PCA/PCR


## Research and Writing

### Boosting Female Labour Force Participation in Singapore
- Competition: MOM-NTU Economics Essay Competition 2025
- Directory: `Female Labour Force Pariticipation`
- Language: R
- Featured: similarity analysis, policy analysis and design

### YouTube as a Carrier of Cultural Heritage (NUS)
- Course: NHS2061 Post-Conflict Cambodia (Year 1-2)
- Directory: `YouTube-and-Culture-in-Cambodia`
- Featured: literature review, research proposal, survey, interview







